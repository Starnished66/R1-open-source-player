 ...
 * must free(). Sources wider or taller than 1200px are rejected (default
 * cover) rather than decoded; this device cannot hold a 2000–4000px RGB888
 * buffer next to the library and audio path. Returns false (leaving
 * *out_pixels untouched) for an unsupported format, an oversized source,
 * or a decode failure. */
