...
static void start_timers(void) {
    /* Live clock is required for Clock, Album Art, and Custom Image modes. */
...
bool gui_lock_screen_show(const gui_lock_screen_options_t * options) {
    ...
    if (current_mode == LOCK_SCREEN_MODE_ALBUM_ART) {
        ...
        /* Show the live centered clock over the album art. */
        update_clock_display();
        ...
        /* Show the live centered clock over the custom image. */
        update_clock_display();
        lv_obj_remove_flag(lock_clock_label, LV_OBJ_FLAG_HIDDEN);
