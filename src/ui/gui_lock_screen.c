...
static void lock_touch_timer_cb(lv_timer_t *timer) {
  ...
  /*
   * The whole screen is the swipe surface, not just a bottom band.
   */
...
static void build_lock_screen_if_needed(void) {
  ...
  /*
   * Clock label is created AFTER the image so it renders above it.
   */
...
bool gui_lock_screen_show(const gui_lock_screen_options_t *options) {
  ...
  /*
   * Reset elements before configuring the selected mode.
   */
  lv_obj_add_flag(lock_image_obj, LV_OBJ_FLAG_HIDDEN);
  ...
  if (current_mode == LOCK_SCREEN_MODE_ALBUM_ART) {
    ...
    /*
     * Album Art lock screen also shows the live centered clock
     * above the image.
     */
    update_clock_display();
    ...
    /*
     * LVGL's lv_fs_get_drv() picks a driver from src[0].
     *
     * A plain POSIX path (what plugin.sd_root() and every
     * plugin-supplied path use) has no registered driver.
     *
     * Only the 'S' POSIX driver is registered, so the path must
     * use the S: prefix.
     *
     * lv_image_set_src() duplicates file-path sources internally,
     * so this stack buffer does not need to outlive this call.
     */
    ...
    /*
     * Custom Image lock screen also shows the live centered clock
     * above the image.
     */
    update_clock_display();
 ...
 *
 * Calling nav_pop() here would read nav_stack[-1], which was the exact
 * out-of-bounds crash class already found and fixed in this reload path
 * for the Plugin Manager.
 *
 * A reload rebuilds the whole screen stack from scratch
 * (gui_navigation_init() loads Home again further down the sequence),
 * so the lock screen does not need to navigate anywhere on its way out.
 *
 * It only needs to release its own resources.
 */
